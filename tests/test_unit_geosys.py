...
POLYGON = (
    "POLYGON((-91.29152885756007 40.39177489815265,-91.28403789132507 40.391776131485386,-91.28386736508233 "
...
class TestGeosys:
    ...
    def test_get_metrics(self, get_response):
        fake_master_data_management_response = mock_http_response_text_content(
            "GET",
            load_data_from_textfile(
                "master_data_management_get_unique_id_mock_http_response"
            ),
        )
        fake_analytics_fabric_response = mock_http_response_text_content(
            "GET", load_data_from_textfile(
                "metrics_lai_radar_mock_http_response")
        )
        get_response.side_effect = [
            fake_master_data_management_response,
            fake_analytics_fabric_response,
        ]
        ...
        df = self.client.get_metrics(
            lai_radar_polygon, schema_id, start_date, end_date)

        assert {"Values.RVI", "Values.LAI",
                "Schema.Id"}.issubset(set(df.columns))
        assert {
    ...
    def test_get_satellite_image_time_series(self, get_response):
        fake_get_tiff_zip_response = mock_http_response_binary_content(
            "GET", load_binary_data_from_zipfile(
                "Refletance_map_mock.tiff.zip")
        )
        fake_image_time_series_response = mock_http_response_text_content(
            "GET",
            load_data_from_textfile(
        ...
        get_response.side_effect = [
            fake_image_time_series_response,
            fake_get_tiff_zip_response,
            fake_get_tiff_zip_response,
        ]
