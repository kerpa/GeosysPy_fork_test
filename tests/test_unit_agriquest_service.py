...
class TestAgriquestService:
    ...
    def test_is_block_for_france(self):
        is_france_block = self.service.is_block_for_france(
    ...
    def test_weather_indicators_builder(self):
        ...
        indicators = self.service.weather_indicators_builder(
            start_date=start_date.date(), end_date=end_date.date(), isFrance=True
        )
        assert set(indicators) == set([3, 4, 5])

        indicators = self.service.weather_indicators_builder(
            start_date=start_date.date(), end_date=end_date.date(), isFrance=False
        )
