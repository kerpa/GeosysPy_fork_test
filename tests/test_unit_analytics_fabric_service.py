...
class TestAnalyticsFabricService:
    ...
    def test_get_metrics(self, get_response):
        ...
        metrics = self.service.get_metrics(
            start_date=start_date,
            end_date=end_date,
            season_field_id="seasonfieldFakeId",
            schema_id="HISTORICAL_HARVEST",
        )
    ...
    def test_get_latest_metrics(self, get_response):
        ...
        metrics = self.service.get_lastest_metrics(
            season_field_id="seasonfieldFakeId", schema_id="HISTORICAL_HARVEST"
        )
    ...
    def test_push_metrics(self, patch_response):
        patch_response.return_value = mock_http_response_text_content(
            "PATCH", load_data_from_textfile(
                "metrics_harvest_mock_http_response")
        )
        data = [{"Timestamp": "2022-01-01", "Values": {"NDVI": 0.5}}]

        result = self.service.push_metrics(
            season_field_id="seasonfieldFakeId",
            schema_id="HISTORICAL_HARVEST",
            values=data,
        )
    ...
    def test_create_schema(self, get_response):
        get_response.return_value = mock_http_response_text_content(
            "POST",
            load_data_from_textfile("create_schema_mock_http_response"),
            status_code=201,
        )
        schema = {"Timestamp": "2021-01-01", "Values": {"NDVI": 0.5}}
        response = self.service.create_schema_id(
            schema_id="NEW_SCHEMA", schema=schema)
