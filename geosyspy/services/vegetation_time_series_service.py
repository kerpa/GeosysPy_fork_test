...
class VegetationTimeSeriesService:
    ...
    def get_modis_time_series(
        ...
        vts_url: str = urljoin(
            self.base_url, GeosysApiEndpoints.VTS_ENDPOINT.value + parameters
        )
    ...
    def get_time_series_by_pixel(
        ...
        parameters: str = f"/values?$offset=0&$limit=None&$count=false&SeasonField.Id={season_field_id}&index={indicator}&$filter=Date >= '{start_date}' and Date <= '{end_date}'"
        vts_url: str = urljoin(
            self.base_url, GeosysApiEndpoints.VTS_BY_PIXEL_ENDPOINT.value + parameters
        )
